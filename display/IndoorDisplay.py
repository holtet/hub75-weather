...
class IndoorDisplay(Display):
    ...
    def __init__(self, config: Config, collection: DataCollection):
        ...
        self.home_image = Image.open("images/heart_house_24.bmp").convert('BGR')
        ...
        for path in os.listdir(dir_path):
            ...
            if os.path.isfile(join):
                self.weather_icons[path] = Image.open(join).convert('BGR')

    def display(self, canvas):
        graphics.DrawLine(canvas, 0, 6, self.config.zs_width, 6, self.grey)
        graphics.DrawLine(canvas, 60, 7, 60, 50, self.grey)
        ...
        temp_text_length = graphics.DrawText(canvas, self.font_6x12, 9, 16, self.green,
        ...
        temp_text_length = graphics.DrawText(canvas, self.font_6x12, 72, 16, self.green,
                                             f'{outdoor.temperature} C')
        graphics.DrawCircle(canvas, temp_text_length + 63, 10, 1, self.green)
        graphics.DrawText(canvas, self.font_6x12, 72, 25, self.green, f'{outdoor.humidity} %')
        # canvas.SetImage(outdoor.weather_icon, 80, 26)
        canvas.SetImage(self.weather_icons[f'{outdoor.icon}.bmp'], 80, 26)
